...
const IndexQuery = () => {
  ...
  const sourceElems = responseSources.map((source) => {
    ...
    return (
      <div key={source.doc_id} className='query__sources__items'>
        <p className='query__sources__items'>{nodeTitle}</p>
        <p className='query__sources__items'>{nodeText}</p>
        <p className='query__sources__items_footer'>
